...
import { DonutChart, BudgetProgressCard } from "../components/ui/Charts";
    ...
    const dataToExport = filteredTransactions.map((transaction) => {
      ...
      const paymentInfo = transaction.paymentMethod
        ? getPaymentMethodInfo(
            ...
            transaction.paymentSubcategory
          )
        : null;
    ...
    <AnimatedPage>
      <div className="p-4 md:p-8 space-y-4 md:space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <h2 className="type-h1 mb-2">keuangan sederhana.</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <Card>
            <div className="p-3 md:p-4 text-center">
              <TrendingUp
                size={18}
                className="md:w-5 md:h-5 mx-auto mb-2 text-green-500"
              />
              <p className="type-label mb-1">pemasukan</p>
              <p className="type-h2 text-green-500">
          ...
          <Card>
            <div className="p-3 md:p-4 text-center">
              <TrendingDown
                size={18}
                className="md:w-5 md:h-5 mx-auto mb-2 text-red-500"
              />
              <p className="type-label mb-1">pengeluaran</p>
              <p className="type-h2 text-red-500">
          ...
          <Card>
            <div className="p-3 md:p-4 text-center">
              <Wallet
                size={18}
                className="md:w-5 md:h-5 mx-auto mb-2 text-(--accent)"
              />
              <p className="type-label mb-1">saldo</p>
              <p
                className={`type-h2 ${
                  balance.balance >= 0 ? "text-(--accent)" : "text-red-500"
                }`}
              >
          ...
          <Card>
            <div className="p-3 md:p-4 text-center">
              <CheckCircle
                ...
                className="md:w-5 md:h-5 mx-auto mb-2 text-(--accent)"
              />
              <p className="type-label mb-1">saving rate</p>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={clsx(
                    "flex-1 py-2 type-body border transition-colors",
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                {categories[formData.type].map((cat) => (
                  <button
                    ...
                    className={clsx(
                      "p-2 md:p-3 text-center border selectable-strong",
                      formData.category === cat.id
                        ? "border-(--accent) bg-(--accent)/10 scale-105"
                        : "border-(--border-color)"
                    )}
                  >
                    <div className="text-xl md:text-2xl mb-1 emoji-bounce">
                      {cat.emoji}
                    </div>
                        ...
                        {paymentMethods
                          ...
                          ?.subcategories.map((sub) => (
                            <button
                              ...
                              className={clsx(
                                "p-2 text-center border selectable-strong",
                                formData.paymentSubcategory === sub.id
                                  ? "border-(--accent) bg-(--accent)/10 scale-105"
                                  : "border-(--border-color)"
                              )}
                            >
                              <div className="text-base md:text-lg mb-1 emoji-bounce">
                                {sub.emoji}
                              </div>
                              <div className="type-caption text-(--text-muted)">
                                {sub.name}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={clsx(
                  "flex items-center gap-2 px-3 md:px-4 py-2 md:py-3 type-body transition-colors whitespace-nowrap",
                  activeTab === tab.id
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <Card>
              <div className="p-4 md:p-6">
                <h3 className="type-label text-(--text-muted) mb-3 md:mb-4 flex items-center gap-2">
                  <Calendar size={14} className="md:w-4 md:h-4" />
                  perbandingan bulan
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div>
                    <p className="type-caption mb-2">bulan ini</p>
                    <p className="type-h2 mb-1">
                    ...
                    >
                      {monthComparison.expenseChange > 0 ? "↑" : "↓"}
                      {Math.abs(monthComparison.expenseChange).toFixed(1)}% dari
                      bulan lalu
                    </p>
                  </div>
                  <div>
                    <p className="type-caption mb-2">rata-rata harian</p>
                    <p className="type-h2">
                      {formatCurrency(avgDailySpending)}
                    </p>
                    <p className="type-caption">pengeluaran per hari</p>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card>
              <div className="p-4 md:p-6">
                <h3 className="type-label text-(--text-muted) mb-3 md:mb-4">
                  kategori terbesar
                </h3>
                {topCategories.length === 0 ? (
                  <p className="type-caption text-center text-(--text-muted) py-4">
                    belum ada data
                  </p>
                ) : (
                  <div className="space-y-2 md:space-y-3">
                    {topCategories.map((cat, index) => (
                      ...
                      >
                        <span className="type-caption text-(--text-muted) w-3 md:w-4">
                          {index + 1}
                        </span>
                        ...
                        <span className="type-caption text-right w-12">
                          {cat.percentage}%
                        </span>
                      </div>
                    ))}
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 md:gap-3">
              <div className="flex-1 relative">
                <Search
                  size={14}
                  className="md:w-4 md:h-4 absolute left-2 md:left-3 top-1/2 -translate-y-1/2 text-(--text-muted)"
                />
                <Input
              ...
              <Card>
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-4">
                    <div>
                      <label className="type-caption text-(--text-muted) mb-2 block">
                        tipe
                      </label>
                      <select
                        value={filters.type}
                        ...
                        className="w-full bg-(--bg-color) border border-(--border-color) p-2 type-body text-(--text-main)"
                      >
            ...
            <Card>
              <div className="p-4 md:p-6">
                ...
                {filteredTransactions.length === 0 ? (
                  <EmptyState
                    type="finance"
                    customMessage="tidak ada transaksi ditemukan"
                  />
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                            <div className="text-lg md:text-2xl shrink-0">
                              {category?.emoji}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-1 md:gap-2 mt-1 flex-wrap">
                                <p className="type-caption text-(--text-muted) whitespace-nowrap">
                                  {new Date(
                                    transaction.date
              ...
              <div className="p-4 md:p-6 space-y-3 md:space-y-4">
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                      {categories.expense.map((cat) => (
                        ...
                        >
                          ...
                          <div className="font-mono text-[10px] md:text-xs text-(--text-muted) mt-1">
                            {cat.name}
                          </div>
                        </button>
                      ))}
                ...
                {Object.keys(budgets).length === 0 ? (
                  <div className="text-center py-8">
                    <Target
                      size={48}
                      className="mx-auto text-(--text-muted) opacity-30 mb-4"
                    />
                  ...
                  <div className="space-y-4">
                    {categories.expense.map((cat) => {
                      ...
                      return (
                        <div
                          key={cat.id}
                          className="p-3 md:p-4 border border-dashed border-(--border-color)"
                        >
                          ...
                          {budgetStatus.isNearLimit &&
                            !budgetStatus.isOverBudget && (
                              <div className="flex items-center gap-1 md:gap-2 text-yellow-500">
                                <AlertTriangle
                                  size={12}
                                  className="md:w-3.5 md:h-3.5 shrink-0"
                                />
                                <p className="font-mono text-[10px] md:text-xs">
                                  mendekati batas! sisa{" "}
        ...
        {activeTab === "analytics" && (
          <div className="space-y-6">
            ...
            <Card>
              <div className="p-4 md:p-6">
                <h3 className="font-mono text-xs md:text-sm uppercase tracking-wider text-(--text-muted) mb-4">
                ...
                {Object.keys(expenseBreakdown).length === 0 ? (
                  ...
                  <DonutChart
                    ...
                    size={180}
                    strokeWidth={35}
                    centerValue={formatCurrency(monthStats.expense)}
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-4 border border-dashed border-(--border-color)">
                    ...
                    >
                      {monthComparison.incomeChange >= 0 ? "↑" : "↓"}
                      {Math.abs(monthComparison.incomeChange).toFixed(1)}% dari
                      bulan lalu
                    </p>
                  </div>
